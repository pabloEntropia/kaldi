...
#
# Mdified by Voctro to process a single audio file. February 2017."
...
if [ $# != 6 ]; then
   echo "Usage: $0 [options] <tgt-data-dir> <src-data-dir> <gmm-dir> <log-dir> <fea-dir> <name>"
   echo "e.g.: $0 data-fmllr/train data/train exp/tri5a exp/make_fmllr_feats/log plp/processed/ mpop1"
   echo ""
...
echo "[VL] This script (make_fmllr_feats_voctro_notransf.sh) does not use  transformation file (.trans) for the DNN forward path for a single speaker not used in the training."
...
# Original using a transformaiton mode .trans
...
# Modified command without the transformation step (Voctro February 2017)
feats="ark,s,cs:apply-cmvn $cmvn_opts --utt2spk=ark:$srcdata/utt2spk scp:$srcdata/cmvn.scp scp:$srcdata/feats.scp ark:- | splice-feats $splice_opts ark:- ark:- | transform-feats --verbose=10 $gmmdir/final.mat ark:- ark:- |"
# transform-feats --utt2spk=ark:$srcdata/utt2spk \"ark:cat $transform_dir/trans.* |\" ark:- ark:- |"
