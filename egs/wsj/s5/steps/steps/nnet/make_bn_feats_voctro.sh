...
#utils/copy_data_dir.sh $srcdata $data; rm $data/{feats,cmvn}.scp 2>/dev/null # Voctro: commented out. it dows not find the cvmn
utils/copy_data_dir_voctro.sh $srcdata $data; rm $data/feats.scp # voctro
...
echo 'All required files are found!'
...
if [ $htk_save == false ]; then
  ...
  # echo NN forward command and arguments (Voctro)
  echo "Printing NN forward command (VOCTRO)"
  echo nnet-forward $nnet_forward_opts --use-gpu=$use_gpu $nnet "$feats"
  echo ark,scp:$bnfeadir/raw_bnfea_$name.1.ark,$bnfeadir/raw_bnfea_$name.1.scp 
