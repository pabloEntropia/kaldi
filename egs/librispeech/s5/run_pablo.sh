...
data=/home/pablo/upf/mt/datasets/LibriSpeech
...
for x in dev_clean test_clean dev_other test_other train_clean_100; do
  python make_mfcc_world_librosa.py data/$x $mfccdir $x $feats_nj
  ...
  for n in $(seq $feats_nj); do
    cat $mfccdir/raw_mfcc_$x.$n.scp || exit 1;
  done > data/$x/feats.scp || exit 1
