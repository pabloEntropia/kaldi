...
. ./cmd_local.sh #  Voctro: changed from cmd.sh to cmd_local.sh
[ -f path.sh ] && . ./path.sh
set -e


# Acoustic model parameters
# numLeavesTri1=2500 # default
numLeavesTri1=131 # voctro for PGP Vooice conversion
numGaussTri1=15000
#numLeavesMLLT=2500
numLeavesMLLT=131 # voctro for PGP Vooice conversion
numGaussMLLT=15000
#numLeavesSAT=2500
numLeavesSAT=131 # voctro for PGP Vooice conversion
numGaussSAT=15000
numGaussUBM=400
numLeavesSGMM=7000
numGaussSGMM=9000
...
#timit=/Users/jjaner/Projects/REBABEL-Neotec/VoiceConv-devel/datasets/TIMIT/TIMIT # @JJ VOCTRO OSX
timit=/home/pablo/upf/mt/datasets/TIMIT/TIMIT # @JJ VOCTRO Linux


# Obsolete: Voctro (skip first stages and run only Karel's DNN)
#doOnlyDNNSteps=true # set to true if all the GMM steps were already generated (~12h of CPU processing on a VM)
#if ! $doOnlyDNNSteps
#then

echo "Voctro: start data preparation and GMM training steps."
...
echo "Voctro: finishing data prepare_lang.sh" # JJ

local/timit_format_data.sh

echo "Voctro: finishing data timit_format_data.sh" # JJ
...
# Voctro: modification
# disabling SGMM steps and go directly to Karel's DNN step (Voctro)
doOnlyKarelDNN=true
if ! $doOnlyKarelDNN
then 
